...
class Directory:
	...
	def get_del_size(self, min_del_size):
		global del_size
		size_files = 0
		for fi in self.files:
			size_files += fi.size
		...
		size_dir = 0
		for key in self.dir_childs:
			size_dir += self.dir_childs[key].get_del_size(min_del_size)
		
		tot_size = size_files + size_dir
		print("dir: {}, size {}".format(self.name, tot_size))
		if tot_size >= min_del_size:
			print("TOT SIZE larger ", tot_size)
			del_size.append(tot_size)
		return tot_size
...
def main():
	...
	print(total_s)
	del_min = total_s - 40000000
	print("Space to delete ", del_min)
	del_s = cur_dir.get_del_size(del_min)
	print(del_size)
